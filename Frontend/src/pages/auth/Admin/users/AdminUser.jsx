...
export default function AdminUsers() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      // Backend kirim array langsung → pakai data
      // Kalau { users: [...] } → pakai data.users
      const userList = Array.isArray(data) ? data : data.users || [];
      
      // Normalisasi nama field ke lowercase
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await fetch(`http://localhost:8080/admin/delete-user/${id}`, { 
        method: "DELETE" 
      });
      fetchUsers();
    } catch (err) {
    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          ...
          { label: "Aktif", value: stats.active, icon: UserCheck, color: "emerald" },
          ...
          <div className="flex-1 max-w-md">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-3 rounded-xl border border-gray-300 focus:outline-none focus:border-emerald-500"
              />
            </div>
          </div>
        </div>
